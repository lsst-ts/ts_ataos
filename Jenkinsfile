    ...
    stages {
        ...
        stage("Build IDL Files") {
            steps {
                script {
                    sh """
                    docker exec -u saluser \${container_name} sh -c \"source ~/.setup.sh && source ~/.bashrc && make_idl_files.py ATAOS ATMCS ATPneumatics ATHexapod ATSpectrograph ATCamera ATPtg ATDome ATDomeTrajectory\"
                    """
                }
            }
        }
        stage("Checkout ts_observatory_control") {
            steps {
                script {
                    sh """
                    docker exec -u saluser \${container_name} sh -c \"source ~/.setup.sh && cd /home/saluser/repos/ && git clone https://github.com/lsst-ts/ts_observatory_control.git && cd /home/saluser/repos/ts_observatory_control && /home/saluser/.checkout_repo.sh \${work_branches} && git pull && eups declare -r . -t saluser\"
                    """
                }
            }
        }
